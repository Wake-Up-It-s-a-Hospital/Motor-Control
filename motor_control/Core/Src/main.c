...
unsigned int sideThreshold = 20; //set side ultrasonic detecting range
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  if (send_flag) {
		  tx_buf[0] = '0' + lost_state;
		  tx_buf[1] = '\0';

		  HAL_UART_Transmit(&huart2, (uint8_t *)tx_buf, 2, HAL_MAX_DELAY);
...
/*	  else if (dis < 1.1 && (theta < -15 || theta > 15)) {
	  ...
	  if(dis > 0.6) // ultrasonic activate distance => up to 0.6m
	  {
		if (Distance1 <= sideThreshold) //side right ultrasonic sensor detected
		{
		   ...
		   PWM1_value = PWM1_value * (Distance1/sideThreshold);
		}
		...
		else if(Distance2 <= frontThreshold)  //front right ultrasonic sensor detected
		{
		   ...
		   PWM1_value = PWM1_value * (Distance2/frontThreshold);
		}
		...
		else if(Distance4 <= sideThreshold) //front left ultrasonic sensor detected
		{
		   ...
		   PWM2_value = PWM2_value * (Distance4/sideThreshold);
		}
		...
		else if(Distance3 <= frontThreshold) //side left ultrasonic sensor detected
		{
		   ...
		   PWM2_value = PWM2_value * (Distance3/frontThreshold);
		}
	  }
